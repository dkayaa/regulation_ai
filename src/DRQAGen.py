...
import dotenv
class DRQAGen(QuestionAnswerGeneratorBase):
    def __init__(self, export_path: str = 'output/default', do_export: bool = True, db=Database(), num_ctx_records: int = 50, ctx_per_record: int = 2, decode_batch_size: int = 1, explainqar_results_fp: str = "", chunk_size: int = 500, k: int = 4):
    ...
    def load(self):
        ...
        t = self.db.get_all_text()

        # Create Document objects from the text string
        docs = [Document(page_content=t, metadata={})]
        pages = docs

        text_splitter = RecursiveCharacterTextSplitter(chunk_size=self.chunk_size, chunk_overlap=self.chunk_size//10)
        ...
        self.retriever = vector_store.as_retriever(search_type="similarity_score_threshold", \
                                                    search_kwargs={"score_threshold": 0.7,"k": self.k})
    ...
    def create_contexts(self) -> DataFrame: 
        ...
        for _, row in explainqar_df.iterrows():
            ...
            r_chain = itemgetter("seed_text") | self.retriever
            context_documents = r_chain.invoke({"seed_text":"\n\n".join(seed_texts)})
            record['contexts'] = [doc.page_content for doc in context_documents]    
